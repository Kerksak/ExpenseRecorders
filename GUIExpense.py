...
GUI = Tk()
GUI.title('โปรแกรมบันทึกค่าใช้จ่าย V.1.0 by Ton')
#GUI.geometry('710x700+500+50')
...
def Save(event=None):
	...
	try:
		...
		dt = days[today] + '-' + dt
		#print(type(transactionid)) # ตรวจสอบว่าตัวแปรเป็นค่าประเภทไหน INTEGER, TEXT/STR
		insert_expense(transactionid,dt,expense,float(price),int(quantity),total) #ต้องแปลงประเภทข้อมูลให้ตรงกับที่เรากำหนดไว้ใน Database ด้วย
...
def UpdateCSV():
	with open('savedata.csv','w',newline='',encoding='utf-8') as f:
		...
		data = list(alltransaction.values()) # แปลง data ให้กลายเป็น list 
...
def UpdateSQL():
	...
	for d in data:
		...
		# d[0] = 202152464264,d[1] = วันเสาร์ 2021-06-26, d[2]'แอปเปิ้ล, d[3]45, d[4]2, d[5]90
...
def DeleteRecord(event=None):
	...
	if check == True:
		...
		del alltransaction[str(transactionid)]  # delete data in dict และเนื่องจาก transactionid เป็นค่าตัวเลข ต้องแปลงเป็น str เพื่อให้ตรงกับค่าที่ select มาจากข้างบน
		#print(alltransaction)
		#UpdateCSV()
		# เปลี่ยนจาก update ใน csv ไป update ใน data base แทน
		delete_expense(str(transactionid)) # delete in database
		update_table()
	else:
		# print('cancel')
		pass
...
def update_table():
	...
	try:
		...
		for d in data:
			...
			alltransaction[d[1]] = d[1:] #d[1] = transactionid #d[1:] คือดึงข้อมูลตั้งแต่ Index 1 (ไม่เอา index 0)
...
################   Righ Click Menu   ######################
def EditRecoed():
	POPUP = Toplevel() # คล้ายๆ กับ TK() เพื่อเพิ่มอีกหน้าต่าง (TK ประกาศได้ครั้งเดียว)
	...
	# สั่งให้ค่าเก่ามาอยู่ในช่องกรอก
...
rightclick.add_command(label='Edit',command=EditRecoed)
...
resulttable.bind('<Button-3>',menupopup) # หากมีการ Click ขวาที่ Resulttable ให้เรียก memupopup
